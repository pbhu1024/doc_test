...
init_qpos: np.ndarray            # 缓存的初始广义坐标（reset_model 可用）
init_qvel: np.ndarray            # 缓存的初始广义速度（reset_model 可用）
...
```python
"""最简 Euler 环境示例 — 离线模式，不需要 OrcaStudio。"""
...
class MyRobotEnv(OrcaGymEulerEnv):
    ...
    def step(self, action: np.ndarray):
        ...
        self.do_simulation(action, self.frame_skip)  # ✅ 走公共 API（自动 sync data）
        obs = self._get_obs()
    ...
    def reset_model(self):
        ...
        self.set_joint_qpos(qpos)    # ✅ 走公共 API（W1 合规写入）
        self.set_joint_qvel(qvel)
        self.mj_forward()            # 更新派生量（body 位姿等）
