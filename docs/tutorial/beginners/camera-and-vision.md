...
OrcaStudio 渲染引擎
  ...
  ├── 渲染 RGB 图像 + 深度图
  ├── H.264 编码
  └── WebSocket 推流 (ws://localhost:<port>)
          │
          ▼
Python 客户端
  └── CameraWrapper
...
class VisionEnv(OrcaGymLocalEnv):
    ...
    def _get_obs(self):
        ...
        """
        # 获取当前相机帧
        image = self._camera.image.copy()  # (H, W, 3) BGR

        return {
            # 本体感知
            "joint_pos": self.data.qpos[:self.nq].copy().astype(np.float32),
            "joint_vel": self.data.qvel[:self.nv].copy().astype(np.float32),
