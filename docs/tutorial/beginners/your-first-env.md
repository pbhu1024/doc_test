...
**推荐使用 `OrcaGymEulerEnv`**（Euler 新主路径），老体系 `OrcaGymLocalEnv` 处于维护模式。
...
| `AttributeError: 'OrcaGymEulerEnv' object has no attribute 'gym'` | 用了老体系 API | Euler 体系没有 `env.gym`，用 `env.data` / `env.model` |
